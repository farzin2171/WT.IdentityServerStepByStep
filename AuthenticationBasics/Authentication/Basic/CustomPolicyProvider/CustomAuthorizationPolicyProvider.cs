...
namespace Basic.CustomPolicyProvider
{
    public class CustomAuthorizationPolicyProvider : DefaultAuthorizationPolicyProvider
    {
        ...
        public override Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            foreach (var customePolicy in DynamicPolicies.Get())
            {
                ...
                {
                    var policy = DynamicAyhorizationPolicyFactory.Create(policyName);
    ...
    public static class DynamicAyhorizationPolicyFactory
    {
        public static AuthorizationPolicy Create(string ploicyName)
        {
            var parts = ploicyName.Split('.');
